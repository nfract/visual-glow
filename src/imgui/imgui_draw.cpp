...
// For backward compatibility: convert all buffers from indexed to de-indexed, in case you cannot render indexed. Note: this is slow and most likely a waste of resources. Always prefer indexed renderer!
...
void    ImFontAtlas::GetTexDataAsRGBA32(unsigned char** out_pixels, int* out_width, int* out_height, int* out_bytes_per_pixel)
{
    ...
    // Although it is likely to be the most commonly used format, our font renderer is 1 channel / 8 bpp
    if (!TexPixelsRGBA32)
    {
...
bool    ImFontAtlasBuildWithStbTruetype(ImFontAtlas* atlas)
{
    ...
    // First font pass: pack all glyphs (no renderer at this point, we are working with rectangles in an infinitely tall texture at this point)
    for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++)
    {
        ImFontConfig& cfg = atlas->ConfigData[input_i];
        ImFontTempBuildData& tmp = tmp_array[input_i];
