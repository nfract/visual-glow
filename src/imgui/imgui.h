...
namespace ImGui
{
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software renderer by setting io.MouseDrawCursor ImGui will render those for you
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // [OBSOLETE] Rendering function, will be automatically called in Render(). Please call your renderer function yourself now! You can obtain the ImDrawData* by calling ImGui::GetDrawData() after Render().
...
// NB- You most likely do NOT need to use draw callbacks just to create your own widget or customized UI renderer (you can poke into the draw list for that)
// Draw callback may be useful for example, A) Change your GPU render state, B) render a complex 3D scene inside a UI element (without an intermediate texture/render target), etc.
// The expected behavior from your renderer function is 'if (cmd.UserCallback != NULL) cmd.UserCallback(parent_list, cmd); else RenderTriangles()'
...
struct ImDrawCmd
{
    ...
    ImDrawCallback  UserCallback;           // If != NULL, call the function instead of renderer the vertices. clip_rect and texture_id will be set normally.
...
// Draw command list
// This is the low-level list of polygons that ImGui functions are filling. At the end of the frame, all command lists are passed to your ImGuiIO::RenderDrawListFn function for renderer.
...
struct ImDrawList
{
    ...
    // Advanced
    IMGUI_API void  AddCallback(ImDrawCallback callback, void* callback_data);  // Your renderer function must check for 'UserCallback' in ImDrawCmd and call the function instead of renderer triangles.
    IMGUI_API void  AddDrawCmd();                                               // This is useful if you need to forcefully create a new draw call (to allow for dependent renderer / blending). Otherwise primitives are merged into the same draw-call as much as possible
...
struct ImDrawData
{
    ...
    IMGUI_API void  DeIndexAllBuffers();                // Helper to convert all buffers from indexed to non-indexed, in case you cannot render indexed. Note: this is slow and most likely a waste of resources. Always prefer indexed renderer!
...
// Sharing a texture for multiple fonts allows us to reduce the number of draw calls during renderer.
// We also add custom graphic data into the texture that serves for ImGui.
//  1. (Optional) Call AddFont*** functions. If you don't call any, the default font will be loaded for you.
//  2. Call GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and retrieve pixels data.
//  3. Upload the pixels data into a texture within your renderer system.
//  4. Call SetTexID(my_tex_id); and pass the pointer/identifier to your texture. This value will be passed back to you during renderer to identify the texture.
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearTexData();             // Clear output texture data (CPU side). Saves RAM once the texture has been copied to renderer memory.
    IMGUI_API void              ClearFonts();               // Clear output font data (glyphs storage, UV coordinates).
    IMGUI_API void              Clear();                    // Clear all input and output.

    // Build atlas, retrieve pixel data.
    // User is in charge of copying the pixels into renderer memory (e.g. create a texture with your engine). Then store your texture handle with SetTexID().
    ...
    ImTextureID                 TexID;              // User data to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during renderer via the ImDrawCmd structure.
    int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your renderer API have texture size restrictions you may want to increase texture width to decrease height.
...
// Font runtime data and renderer
...
struct ImFont
{
    ...
    ImVec2                      DisplayOffset;      // = (0.f,0.f)  // Offset font renderer by xx pixels
    ...
    // 'max_width' stops renderer after a certain width (could be turned into a 2d size). FLT_MAX to disable.
