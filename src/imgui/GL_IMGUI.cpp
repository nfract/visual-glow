...
/*
 ...
 - All renderer information are stored into command-lists until ImGui::Render() is called.
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // We are using scissoring to clip some objects. All low-level renderer API supports it.
 ...
 - 2018/02/16 (1.60) - obsoleted the io.RenderDrawListsFn callback, you can call your renderer engine render function after ImGui::Render(). Use ImGui::GetDrawData() to retrieve the ImDrawData* to display.
 ...
 - 2015/07/08 (1.43) - switched renderer data to use indexed renderer. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
                       this necessary change will break your renderer function! the fix should be very easy. sorry for that :(
                     ...
                     - if you REALLY cannot render indexed primitives, you can call the draw_data->DeIndexAllBuffers() method to de-index the buffers. This is slow and a waste of CPU/GPU. Prefer using indexed renderer!
 ...
 - 2014/08/28 (1.09) - changed the behavior of IO.PixelCenterOffset following various renderer fixes
...
void ImGui::NewFrame()
{
    ...
    // Mark renderer data as invalid to prevent user who may have a handle on it to use it
    g.DrawData.Clear();
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    // Align whole block. We should defer that to the better renderer function when we'll have support for individual line alignment.
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
        ...
        // We also hide the window from renderer because we've already added its border to the command list.
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        // We are attempting to do most of that in **one main pass** to minimize the computation cost (non-negligible for large amount of text) + 2nd pass for selection renderer (we could merge them by an extra refactoring effort)
